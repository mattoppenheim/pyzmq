...
from vispy_graph import VispyGraph
import threading
...
class WitiltTk(Frame):
    ...
    def __init__(self, parent):
        ...
        witilt_thread.start()
        vispy_thread = threading.Thread(target=self.start_vispy)
        vispy_thread.start()
        self.initUI()
...
#             out_str += '{}:{:02x} '.format(key, value)
    ...
    def start_vispy(self):
        ''' Instigate the vispy graph for data display. '''
        self.vispy_graph = VispyGraph()
